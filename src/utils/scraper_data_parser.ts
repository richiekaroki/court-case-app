...
const regex =
  /\b(?:\d+(?:st|nd|rd|th)(?:\s+to\s+\d+(?:st|nd|rd|th))?(?:,?\s+and\s+\d+(?:st|nd|rd|th)(?:\s+to\s+\d+(?:st|nd|rd|th))?)?\s+Respondents?)\b/gi;
  ...
  if (courts) {
    ...
    for (const court of courts.courts) {
      ...
      if (
        !specialCourt &&
        existingCounty !== null &&
        existingCounty !== undefined &&
        existingCounty.get("id") !== undefined
      ) {
        ...
        if (created && existingCourt) {
          console.log(
            `Court ${courtName} created with ID : ${existingCourt.get("id")}`,
          );
        } else {
          console.log(
            `Court ${courtName} already exists with ID : ${existingCourt.get("id")}`,
          );
        }
        ...
        if (courtCreateResult) {
          ...
          if (created) {
            console.log(
              `Special court ${courtName} created with ID : ${existingCourt?.get("id")}`,
            );
          } else {
            console.log(
              `Special court ${courtName} already exists with ID : ${existingCourt?.get("id")}`,
            );
          }
      ...
      try {
        ...
        if (!specialCourt && countyCreateObj) {
          ...
          if (
            ...
            console.log(
              `County ${county} created with ID : ${existingCounty.get("id")}`,
            );
          } else if (
            !created &&
            existingCounty &&
            ...
            console.log(
              `County ${county} already exists with ID : ${existingCounty.get("id")}`,
            );
          } else {
    ...
    try {
      ...
      if (caseMetaData_.judges) {
        ...
        const createJudge = async (
          judgeName: string,
        ): Promise<{ judgeObj: Judge; created: boolean; id: number }> => {
          const sanitizedName = judgeName
            .trim()
            .replace(/\s+/g, " ")
            .normalize("NFC");
          console.log(`Attempting to create judge ${sanitizedName}`);

          let judgeInstance = await Judge.findOne({
        ...
        if (judges.includes(",")) {
          ...
          for (const judgeName of judgeNames) {
            try {
              ...
              if (
                !judgeCreated.created &&
                (!judgeCreated.judgeObj ||
                  judgeCreated.judgeObj === null ||
                  judgeCreated.judgeObj === undefined)
              ) {
                ...
                if (judgeCreated.judgeObj && judgeCreated.created) {
                  ...
                  console.log(
                    `Created judge ${judgeCreated.judgeObj.get("name")} successfully with id ${judgeCreated.id}`,
                  );
                  judgeIds.push(judgeCreated.id);
                } else if (judgeCreated.judgeObj && !judgeCreated.created) {
                  console.log(
                    `Judge ${judgeName} already exists with id : ${judgeCreated.id}`,
                  );
          ...
          try {
            ...
            if (
              ...
              if (judgeCreated.judgeObj && judgeCreated.created) {
                ...
                console.log(
                  `Judge ${judges} already exists with id : ${judgeCreated.id}`,
                );
                judgeIds.push(judgeCreated.id);
              } else {
      ...
      if (caseMetaData_.advocates) {
        ...
        const createAdvocate = async (
          advocateName: string,
          type: string,
        ): Promise<{ advocateObj: Advocate; created: boolean; id: number }> => {
          const sanitizedName = advocateName
            .trim()
            .replace(/\s+/g, " ")
            .normalize("NFC");
          console.log(`Attempting to create advocate ${sanitizedName}`);

          let advocateInstance = await Advocate.findOne({
            where: { name: sanitizedName },
          });
          ...
          if (!advocateInstance) {
            ...
            return {
              ...
              created: true,
              id: advocateInstance.id,
            };
          } else {
            return {
              advocateObj: advocateInstance,
              created: false,
              id: advocateInstance.get("id"),
            };
          }
        };
        ...
        for (let advocate of advocates) {
          try {
            const advocateCreated: any = await createAdvocate(
              advocate.name,
              advocate.type,
            );

            if (
              !advocateCreated.created &&
              (!advocateCreated.advocateObj ||
                advocateCreated.advocateObj === null ||
                advocateCreated.advocateObj === undefined)
            ) {
              ...
              if (advocateCreated.advocateObj && advocateCreated.created) {
                console.log(
                  `Created advocate ${advocateCreated.advocateObj.get("name")} successfully`,
                );
                advocateIds.push(advocateCreated.id);
              } else if (advocateCreated.judgeObj && !advocateCreated.created) {
                console.log(
                  `Advocate ${advocateCreated} already exists with id : ${advocateCreated.id}`,
                );
                advocateIds.push(advocateCreated.id);
              } else {
      ...
      let caseInstance = await Case.findOne({
        where: { title: caseMetaData_.title },
      });
      let wasCreated;
      ...
      if (wasCreated) {
        console.log(
          `Created case title ${caseInstance.get("title")} successfully`,
        );
      } else {
      ...
      if (judgeIds.length > 0 && wasCreated && caseInstance) {
        ...
        for (const judgeId of judgeIds) {
          ...
          if (!caseJudgeInstance) {
            ...
            console.log(
              `CaseJudge record created : ${JSON.stringify(caseJudgeInstance)}`,
            );
          } else {
          ...
          if (
            !caseJudgeWasCreated &&
            (!caseJudgeInstance ||
              caseJudgeInstance === null ||
              caseJudgeInstance === undefined)
          ) {
            console.log(`Failed to create case judge record`);
          } else {
            if (caseJudgeInstance && caseJudgeWasCreated) {
              console.log(
                `Case judge instance was created successfully with id ${caseJudgeInstance.id}`,
              );
            } else if (caseJudgeInstance && !caseJudgeWasCreated) {
              console.log(
                `Case judge record was already existant with id ${caseJudgeInstance.id}`,
              );
            } else {
      ...
      if (advocateIds.length > 0 && wasCreated && caseInstance) {
        ...
        for (const advocateId of advocateIds) {
          ...
          let caseAdvocateInstance: CaseAdvocate | null =
            await CaseAdvocate.findOne({
              where: {
                [Op.and]: [
          ...
          if (!caseAdvocateInstance) {
            ...
            console.log(
              `CaseAdvocate record created : ${JSON.stringify(caseAdvocateInstance)}`,
            );
          } else {
            console.log(`CaseJudge instance already exists`);
          ...
          if (
            !caseAdvocateWasCreated &&
            (!caseAdvocateInstance ||
              caseAdvocateInstance === null ||
              caseAdvocateInstance === undefined)
          ) {
            ...
            if (caseAdvocateInstance && caseAdvocateWasCreated) {
              ...
              console.log(
                `Case advocate record was alreay existant with id ${caseAdvocateInstance.id}`,
              );
            } else {
